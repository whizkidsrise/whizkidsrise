...
class User2(UserMixin, ResourceMixin, db.Model):
    ROLE = OrderedDict([
        ('member', 'Member'),
        ('admin', 'Admin')
    ])
    ...
    email = db.Column(db.String(255), unique=True, index=True, nullable=False,
                      server_default='')
    skills = db.Column(db.String(4001), unique=False, server_default='')
    train = db.Column(db.String(4001), unique=False, nullable=False,
                      server_default='')
    whizcoin = db.Column(db.Integer, nullable=False, default=0)
    ...
    enddate = db.Column(db.String(255), unique=False, index=False,
                      nullable=False, server_default='')
    interesting_projects = db.Column(db.String(4100), unique=False, server_default='')
    fullname = db.Column(db.String(255), unique=False, index=False,
                      nullable=False, server_default='')
    current_project = db.Column(db.String(255), unique=False, index=False,
                      nullable=False, server_default='')
    ...
    def __init__(self, **kwargs):
        ...
        self.password = User2.encrypt_password(kwargs.get('password', ''))
    ...
    def find_by_identity(cls, identity):
        ...
        """
        return User2.query.filter(
          (User2.email == identity) | (User2.username == identity)).first()
    ...
    def deserialize_token(cls, token):
        ...
        try:
            ...
            return User2.find_by_identity(decoded_payload.get('user_email'))
        except Exception:
            return None
    ...
    def initialize_password_reset(cls, identity):
        ...
        :return: User instance
        """
        u = User2.find_by_identity(identity)
        reset_token = u.serialize_token()

        # This prevents circular imports.
        from snakeeyes.blueprints.user2.tasks import (
